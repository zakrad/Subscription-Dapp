...
constructor() ERC1155("") {
    ...
    function ifExpired(uint id) internal view returns(bool) {
        if(subscribers[msg.sender].plan == id) {
            if(block.timestamp - subscribers[msg.sender].date < plans[id].time) {
                return false;
            } else {
                return true;
            }
    ...
    function addPlan(string memory _name, uint256 price, uint time) external onlyOwner {
        console.log("totalPlans is %o", totalPlans);
        totalPlans = totalPlans + 1;
        uint256 id = totalPlans;
        plans[id] = plan(msg.sender, _name, 0, price, time);
        console.log("totalPlans is %o and id is %o", totalPlans, id);
    }

    function updatePlan(uint id, string memory _name, uint256 price, uint time) external onlyOwner {
        plans[id] = plan(msg.sender ,_name, plans[id].subscribers, price, time);
    }
